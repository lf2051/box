...
const pushBairrosToBot = async (bairros) => {
  ...
  try {
    await fetch(baseUrl + '/bairros', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ bairros }),
    });
  } catch (error) {
    console.warn('Falha ao enviar bairros para o bot WhatsApp:', error);
  }
};
...
const pushAppInfoToBot = async (appInfo) => {
  ...
  try {
    ...
    console.warn('Falha ao enviar configuracoes para o bot WhatsApp:', error);
  }
};
...
export const saveBairrosEntrega = async (userId, bairros) => {
  ...
  await pushBairrosToBot(bairros);
  return result;
};
...
export const createChatbotApi = (userId) => ({
  ...
  async getTaxaEntrega(bairro) {
    const snapshot = await syncDeliverySnapshot(userId);
    const bairros = snapshot.settings?.bairros || [];
    const search = String(bairro || '').trim().toLowerCase();
    const found = search
      ? bairros.find((item) => String(item.nome || '').toLowerCase() === search)
      : null;

    if (!search) {
      return {
        status: 'success',
        message: 'Informe o nome do bairro para consultar a taxa.',
        bairros,
      };
    }
    ...
    return {
      ...
      bairro: found || null,
      message: found
        ? `Taxa de entrega para ${found.nome}: ${formatCurrency(found.taxaEntrega)}.`
  ...
  async getHorarioFuncionamento() {
    ...
    return {
      status: 'success',
      horario: snapshot.settings?.appInfo?.horarioFuncionamento || '',
    };
  },
  async getEnderecoLoja() {
    const snapshot = await syncDeliverySnapshot(userId);
    return {
      status: 'success',
      endereco: snapshot.settings?.appInfo?.enderecoLoja || '',
    };
  },
