...
import { X, Edit2, RefreshCw, Plus } from 'lucide-react';
...
const EditProductModal = ({ isOpen, onClose, product, onSave }) => {
  ...
  useEffect(() => {
    if (product && isOpen && user) {
      ...
      setFormData({
        ...
        tipo: product.tipo || (product.eh_combo ? 'combo' : 'simples'),
        eh_combo: product.eh_combo || product.tipo === 'combo',
        insumos: []
      });

      // 2. Fetch all products for insumo selection
      fetchProducts();

      // 3. If combo, fetch existing insumos
      if (product.tipo === 'combo' || product.eh_combo) {
        loadInsumos(product.id);
      }
    }
  }, [product, isOpen, user]);
  ...
  const loadInsumos = async (comboId) => {
    const insumos = await fetchComboInsumos(comboId);
    if (insumos) {
      setFormData(prev => ({
        ...prev,
        insumos: insumos.map(i => ({
           insumo_id: i.insumo_id,
           nomeProduto: i.produto?.descricao,
  ...
  const handleGenerateBarcode = async () => {
    ...
    try {
      ...
      if (exists) {
        ...
        toast({ title: 'Código gerado', description: newCode });
      }
    } catch (error) {
      console.error(error);
    } finally {
  ...
  const handleAddInsumo = (insumo) => {
    if (formData.insumos.some(i => i.insumo_id === insumo.insumo_id)) {
      toast({ title: 'Insumo já adicionado', variant: 'destructive' });
      return;
    }
    setFormData(prev => ({ ...prev, insumos: [...prev.insumos, insumo] }));
  };

  const handleRemoveInsumo = (insumoId) => {
    setFormData(prev => ({ ...prev, insumos: prev.insumos.filter(i => i.insumo_id !== insumoId) }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.tipo === 'combo' && formData.insumos.length === 0) {
      toast({ title: 'Adicione pelo menos 1 insumo', variant: 'destructive' });
      return;
    }
    ...
    const success = await onSave(product.id, {
      ...
      estoque: formData.tipo === 'combo' ? 0 : (parseInt(formData.estoque) || 0),
            ...
            <form id="editProductForm" onSubmit={handleSubmit} className="space-y-6">
              ...
              {formData.tipo === 'combo' && (
                <div className="space-y-4 animate-in slide-in-from-top-4 duration-300">
                  ...
                  <ComboInsumoTable insumos={formData.insumos} onRemove={handleRemoveInsumo} />
                </div>
              )}
